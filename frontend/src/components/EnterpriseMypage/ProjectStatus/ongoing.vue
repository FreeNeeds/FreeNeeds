<template>
  <div>
    <div class="carousel-wrapper-mine mx-auto mt-4 carouselWrapperMyPageCompanyRecruit" data-bs-touch="false" id="ongoingCarouselWrapperMyPageCompanyRecruit">
      ...
      createInstance().get('/project/company/' + String(this.$store.state.accounts.loginUserInfo.id)).then(res => {
          for (let i = 0; i < res.data.length; i++) {
            let remainDateTmp = Math.ceil(
              (new Date(res.data[i].startDate).getTime() - new Date().getTime()) /
                (1000 * 60 * 60 * 24) - 1
            );
            if (remainDateTmp < 0) {
